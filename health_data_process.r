...
corrupt <- map(all_data, ~ keep(.x, ~ any(grepl("45001", colnames(.)))))
...
covariates <- covariates_wdi %>%
  left_join(covariates_wgi, by = c("country", "year")) %>%
  filter(!is.na(gdp_pc))
...
rn_45 <- recoverNetwork(data = data_balanced, lambda = lambda_vec, exoeffects = 1)

# Estrazione Risultati (come prima)
W_est_45 <- rn_45$unpenalisedgmm$W
cat("\n--- Risultati Stima ---\n")
cat("Rho (Endogenous Effect):", rn_45$unpenalisedgmm$rho, "\n")
cat("\nMatrice W (Primi 5 nodi) -> Dovrebbe essere la tua soluzione!\n")
print(W_est_45[1:5, 1:5])
rho_45 <- rn_45$unpenalisedgmm$rho
saveRDS(W_est_45, file = "matrice_W_stima_45001.rds")
saveRDS(rho_45, file = "rn_45$unpenalisedgmm$rho.rds")
