...
Appendix (Table~\ref{tab:tab:grid_search}). The associated structural estimates
...
distributions underlying these layouts are given in Appendix
(Figure~\ref{fig:degree_hist}).
Both networks come directly row-normalised from the estimation, where the condition is set in order to retrieve the peer effect. This transformation, together with the sparsity of the recovered network results in most of the non-zero weights being 1s. This is particularly evident in the Appendix figure \ref{fig:degree_hist}, where a degree distribution for the two networks is reported.
% =============================================================================
...
\begin{figure}[H]
    \centering
    \caption{$R^2$, fully lagged peer effects}
    \label{fig:bdf_r2_fullag}
    \includegraphics[width=0.85\textwidth]{assets/figures/bdf_r2_fullag.pdf}
    \par\vspace{0.5em}
    {\footnotesize \textit{Notes.} Within-$R^2$ of the fully-lagged specification (eq.~\ref{eq:fullag}), estimated by OLS with country and year fixed effects; the contemporaneous peer term is excluded, so no instrument is used. Because these are OLS lagged-model fits rather than the IV simultaneous fits of Figure~\ref{fig:bdf_r2_nolag}, the values are comparable across networks \emph{within this figure} but not directly against the IV within-$R^2$ reported there. \textit{Observations:} Quality $N\times T = 572$; Environment $N\times T = 416$.}
\end{figure}
