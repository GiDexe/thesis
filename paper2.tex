...
The dyadic regressions in Table~\ref{tab:dyadic_recovered_fp_fe} ask what predicts the presence and strength of a recovered tie, with sender and receiver fixed effects absorbing all monadic heterogeneity in each country's propensity to influence or to be influenced \citep{dzemski2019}. This is the paper's sharpest finding.
...
Some limitations are worth stating explicitly.
...
The second concerns the network-recovery methodology and data dimensions. The \citet{depaula2025} estimator is consistent as $T$ grows for fixed $N$; with $T = 25$ for Quality and $T = 21$ for Environment, both panels are short relative to the ideal, and the Environment panel is the more binding constraint. The weaker peer-effect identification and the wider GMM confidence interval for that standard are consistent with this. Extending the sample beyond 2017 would require a harmonisation of the pre- and post-2018 ISO Survey methodologies that is not currently feasible. Furthermore, the recovery methodology itself imposes necessary but restrictive structural assumptions. It requires the latent network to be fixed over the entire sample period; given that our panel spans over two decades, this static assumption precludes the possibility that channels of influence evolved as European integration deepened. Additionally, to regularize the high-dimensional parameter space, the adaptive elastic-net algorithm imposes a strict sparsity penalty. While mathematically necessary for consistency in short panels, it mechanically forces weak ties to zero, meaning our recovered networks capture only the most dominant channels of diffusion rather than the complete web of low-intensity spillovers.

The third is the restriction of the sample to the 27 EU member states, which presents a specific tradeoff regarding unobserved confounders. On one hand, this homogeneity is an asset: it limits unobserved institutional heterogeneity and ensures that the regulatory baseline is highly comparable across nodes. On the other hand, it introduces the risk of unobserved regional confounders. The EU is a highly active regulatory environment, and coordinated policy initiatives (such as European environmental directives or supply-chain mandates) may hit sub-clusters of member states at similar times, moving several countries together. While country and year fixed effects absorb uniform pan-European shocks, localized EU regulatory waves could induce synchronous adoption patterns that the recovery algorithm inadvertently attributes to peer-to-peer diffusion. The fully lagged specification provides a further check by closing the contemporaneous channel entirely. Neither device is watertight, however, and the recovered networks should be read as the structure of conditional covariation in adoption dynamics rather than as a causal map of deliberate influence.

A final econometric limitation concerns the application of the \citet{bramoulle2009} identification strategy to panel data. The use of intransitive peers' characteristics ($W^{2}X$ and $W^{3}X$) as excluded instruments is fundamentally rooted in cross-sectional network topology. While its application to panel data with two-way fixed effects is standard in the applied literature, it relies on the strong assumption of strict exogeneity of covariates across time and precludes complex dynamic feedback loops between adoption and shifting economic characteristics.
