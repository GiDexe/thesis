...
For Environment the evidence speaks to how the network \emph{forms} rather than how strongly it transmits. The dyadic results show that ties arise between countries with asymmetric environmental-policy stances but comparable economies, a pattern consistent with adoption operating as a signalling device toward international audiences and more valuable where domestic governance is weaker \citep{berliner2013}. The hierarchical topology and the dyadic regression results that identify as links' predictors economic distance and evironmental policy asymmetry are consistent with what such a signalling logic would predict, with adoption oriented toward a small number of focal reference countries rather than exchanged reciprocally between peers. Whether adoption then propagates strongly along this structure is a separate question that the shorter Environment panel leaves unresolved: the recovered peer effect is small and imprecisely estimated, so we can characterise the structure of the Environment network and what shapes its edges without claiming a precisely identified intensity of transmission.
