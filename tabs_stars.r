...
get_stars <- function(val, se) {
  tstat <- abs(val / se)
  if (tstat > qnorm(0.995)) {
    return("***")
  }
  if (tstat > qnorm(0.975)) {
    return("**")
  }
  if (tstat > qnorm(0.950)) {
    return("*")
  }
  return("")
}
...
get_se <- function(r) {
  av <- r$unpenalisedgmm$asyvar
  se <- sqrt(abs(av[1:(2 * K + 1)]))
  list(rho = se[1], beta = se[2:(K + 1)], gamma = se[(K + 2):(2 * K + 1)])
}

fmt_with_stars <- function(val, se) {
  paste0(fmt(val), get_stars(val, se))
}
...
writeLines(sprintf(
  "$\\hat{\\rho}$ & %s & %s & %s & %s \\\\",
  fmt_with_stars(r37$unpenalisedgmm$rho, se37$rho),
  fmt_with_stars(r45$unpenalisedgmm$rho, se45$rho),
  fmt_with_stars(r37s$unpenalisedgmm$rho, se37s$rho),
  fmt_with_stars(r45s$unpenalisedgmm$rho, se45s$rho)
), t1)

# rho SE
writeLines(sprintf(
  " & %s & %s & %s & %s \\\\",
  fmt_se(se37$rho), fmt_se(se45$rho),
  fmt_se(se37s$rho), fmt_se(se45s$rho)
), t1)
...
for (i in 1:K) {
  writeLines(sprintf(
    ...
    covnames[i],
    fmt_with_stars(r37$unpenalisedgmm$beta[i], se37$beta[i]),
    fmt_with_stars(r45$unpenalisedgmm$beta[i], se45$beta[i]),
    fmt_with_stars(r37s$unpenalisedgmm$beta[i], se37s$beta[i]),
    fmt_with_stars(r45s$unpenalisedgmm$beta[i], se45s$beta[i])
  ), t1)
  writeLines(sprintf(
    " & %s & %s & %s & %s \\\\",
    fmt_se(se37$beta[i]), fmt_se(se45$beta[i]),
    fmt_se(se37s$beta[i]), fmt_se(se45s$beta[i])
  ), t1)
}
...
for (i in 1:K) {
  writeLines(sprintf(
    ...
    covnames[i],
    fmt_with_stars(r37$unpenalisedgmm$gamma[i], se37$gamma[i]),
    fmt_with_stars(r45$unpenalisedgmm$gamma[i], se45$gamma[i]),
    fmt_with_stars(r37s$unpenalisedgmm$gamma[i], se37s$gamma[i]),
    fmt_with_stars(r45s$unpenalisedgmm$gamma[i], se45s$gamma[i])
  ), t1)
  writeLines(sprintf(
    " & %s & %s & %s & %s \\\\",
    fmt_se(se37$gamma[i]), fmt_se(se45$gamma[i]),
    fmt_se(se37s$gamma[i]), fmt_se(se45s$gamma[i])
  ), t1)
}
